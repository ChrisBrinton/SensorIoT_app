...
const initialState = {
  ...
  nodeData: [],
  /* test data
  nodeData: [ { gateway_id: "111111",
                latest: [ { nodeID: 1, latestData: { F: 65.1, H: 45.5, P: 29.8, BAT: 2.99, RSSI: -25 }, nodeTime: dateFns.setHours(new Date(2018, 1, 2), 6) },
              ...
              { gateway_id: "222222",
                latest: [ { nodeID: 4, latestData: { F: 65.1, H: 45.5, P: 29.8, BAT: 2.99, RSSI: -25 }, nodeTime: dateFns.setHours(new Date(2018, 1, 2), 6) }
                        ]
              }
            ]
  */
  ...
  switch (action.type) {
    ...
    case "RECEIVE_NODE_LATEST":
      ...
      for (let i in action.json) {
        let newData = [];
        for (let j in action.json[i].latest) {
          unpackLatestData(newData, action.json[i].latest[j]);
        }
        let obj = { gateway_id: action.json[i].gateway_id, latest: newData };
        newNodeData.push(obj);
      }
      //console.log("receiveSensorData newNodeData", newNodeData);
